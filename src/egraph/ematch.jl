...
function ematch_compile(p, pvars, direction)
  ...
  quote
    function ($(gensym("ematcher")))(
      g::$(EGraphRewriting.EGraph),
      rule_idx::Int,
      root_id::$(EGraphRewriting.Id),
      stack::$(EGraphRewriting.OptBuffer){UInt16},
      ematch_buffer::$(EGraphRewriting.OptBuffer){UInt64},
    )::Int
...
function check_constant_exprs!(buf, pat::Pat)
  if pat.type === PAT_LITERAL
    push!(buf, :($(EGraphRewriting.has_constant)(g, $(pat.head_hash)) || return 0))
  elseif pat.type === PAT_EXPR
    if !(pat.head isa Pat)
      push!(buf, :($(EGraphRewriting.has_constant)(g, $(pat.head_hash)) || $(EGraphRewriting.has_constant)(g, $(pat.name_hash)) || return 0))
    end
...
function check_var_expr(addr::Int, predicate::Base.Fix2{typeof(isa),<:Type}, idx::Int64)
  quote
    ...
    if $(Symbol(:enode_idx, addr)) <= eclass_length
      ...
      if !$(v_isexpr)(n)
        h = $(v_head)(n)
        hn = $(EGraphRewriting.get_constant)(g, h)
        if $(predicate)(hn)
          $(Symbol(:enode_idx, addr)) += 1
...
function lookup_expr(addr::Int, p::Pat)
  ...
  quote
    ecid = $(EGraphRewriting.lookup_pat)(g, $p)
    if ecid > 0
      $(Symbol(:σ, addr)) = ecid
      pc += 0x0001
      @goto compute
